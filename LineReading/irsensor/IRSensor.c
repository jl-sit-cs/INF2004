...
uint16_t moving_average(uint16_t new_value) {
    ...
    bool is_black = (new_value > 400); // Black line detected
...
void loop() {
    ...
    if (filtered_value > 400) { // ADC > 400  (black line detected)
        if (!measuring_black) {
            start_time = time_us_64(); // Start the timer for black line
        ...
        if (measuring_black) {
            end_time = time_us_64(); // Stop the timer for black line
            uint64_t pulse_width_black = end_time - start_time; // Calculate pulse width for black line
        ...
        if (!measuring_white) {
            start_time = time_us_64(); // Start the timer for white surface
    ...
    if (measuring_white && filtered_value > 400) {
        end_time = time_us_64(); // Stop the timer for white surface
        uint64_t pulse_width_white = end_time - start_time; // Calculate pulse width for white surface
